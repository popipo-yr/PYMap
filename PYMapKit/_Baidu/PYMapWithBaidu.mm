...
#import "PYBaiduAnnotationView.h"
...
- (void)addCircleWithCenterCoordinate:(CLLocationCoordinate2D)coord
                          ...
                          strokeColor:(UIColor *)strokeColor
                            ...
                            lineWidth:(CGFloat)lineWidth
                                  uid:(NSString*)uid
{
    BMKCircle* acircle = [BMKCircle circleWithCenterCoordinate:coord
                                                        radius:radius];
    acircle._uid_ = uid;
    ...
    if ([overlay isKindOfClass:[BMKShape class]]) {
       ...
       PYShapeInfo *shapeInfo = [_shapeCache objectForKey:uid];
        ...
        if (shapeInfo && ShapeType_Polygon == shapeInfo.shapeType) {
            ...
            if ([shape isKindOfClass:[BMKCircle class]]) {
                
                BMKCircleView *view = [[BMKCircleView alloc] initWithCircle:(BMKCircle*)shape];
...
- (BMKAnnotationView *)mapView:(BMKMapView *)mapView viewForAnnotation:(id <BMKAnnotation>)annotation
{
    if ([annotation isKindOfClass:[BMKPointAnnotation class]]) {
        ...
        PYBaiduAnnotationView *annotationView = (PYBaiduAnnotationView *)[mapView dequeueReusableAnnotationViewWithIdentifier:reuseId];

        if (annotationView == nil) {
            annotationView = [[PYBaiduAnnotationView alloc] initWithAnnotation:annotation reuseIdentifier:reuseId];
        }
...
{
    if (![view isKindOfClass:[PYBaiduAnnotationView class]]) return;

    if ([_mapDelegate respondsToSelector:@selector(pyMap:annotationSelectAtUid:)]) {
        [_mapDelegate pyMap:self annotationSelectAtUid:((PYBaiduAnnotationView *)view).other];
    }
}
...
- (void)mapView:(BMKMapView *)mapView didDeselectAnnotationView:(BMKAnnotationView *)view
{
    ...
    if ([_mapDelegate respondsToSelector:@selector(pyMap:annotationDeSelectAtUid:)]) {
        [_mapDelegate pyMap:self annotationDeSelectAtUid:((PYBaiduAnnotationView *)view).other];
    }
}
...
- (void)updateCallout
{
    for (BMKPointAnnotation *aAnnotation in _mapView.annotations) {
        ...
        PYBaiduAnnotationView *annotationView = (PYBaiduAnnotationView *)[_mapView viewForAnnotation:aAnnotation];
        if (![annotationView isKindOfClass:[PYBaiduAnnotationView class]]) continue;

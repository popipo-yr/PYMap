...
- (void)addCircleWithCenterCoordinate:(CLLocationCoordinate2D)coord
                          ...
                          strokeColor:(UIColor *)strokeColor
                            ...
                            lineWidth:(CGFloat)lineWidth
                                  uid:(NSString*)uid
{
    QCircle* acircle = [QCircle circleWithCenterCoordinate:coord
                                                        radius:radius];
    acircle._uid_ = uid;
...
- (QOverlayView *)mapView:(QMapView *)mapView viewForOverlay:(id <QOverlay>)overlay
{
    if ([overlay isKindOfClass:[QShape class]]) {
        ...
        if (shapeInfo && ShapeType_Polygon == shapeInfo.shapeType) {
            ...
            if ([shape isKindOfClass:[QCircleView class]]) {
                
                QCircleView *view = [[QCircleView alloc] initWithCircle:(QCircle*)shape];
...
- (QAnnotationView *)mapView:(QMapView *)mapView viewForAnnotation:(id <QAnnotation>)annotation
{
    if ([annotation isKindOfClass:[QPointAnnotation class]]) {
        ...
        annotationView.centerOffset = CGPointMake(0,
                                                  -annotationView.bounds.size.height * 0.5f);
...
-(void)mapView:(QMapView *)mapView didSelectAnnotationView:(QAnnotationView *)view{
    ...
    if ([_mapDelegate respondsToSelector:@selector(pyMap:annotationSelectAtUid:)]) {
        [_mapDelegate pyMap:self annotationSelectAtUid:((PYTencentAnnotationView*)view).other];
    }
}
...
-(void)mapView:(QMapView *)mapView didDeselectAnnotationView:(QAnnotationView *)view{
    ...
    if ([_mapDelegate respondsToSelector:@selector(pyMap:annotationDeSelectAtUid:)]) {
        [_mapDelegate pyMap:self annotationDeSelectAtUid:((PYTencentAnnotationView*)view).other];
    }
...
{
    QShape* shape = [_shapeCache objectForKey:uid].shape;
    if ([shape conformsToProtocol:@protocol(QOverlay)]) {
        [_mapView removeOverlay:(id <QOverlay>)shape];
    }
